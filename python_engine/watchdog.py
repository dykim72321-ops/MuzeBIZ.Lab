...
def run_watchdog():
    send_alert(f"🐕 [Watchdog] 생존 방어 봇 가동 완료. (Max Loss Trigger: {MAX_DAILY_LOSS_PCT}%)")
    while True:
        try:
            acc = get_account()
            ...
            if last_equity > 0:
                ...
                if daily_pnl_pct <= MAX_DAILY_LOSS_PCT:
                    send_alert(f"🚨 [FATAL ERROR] 엣지 붕괴 감지! 일간 손실 한도 초과 ({daily_pnl_pct:.2f}% <= {MAX_DAILY_LOSS_PCT}%).")
                    send_alert(f"💀 [Watchdog] 모든 대기 주문 취소 및 전량 시장가 청산(Liquidate)을 즉시 집행합니다.")
                    
                    cancel_all_orders()
                    liquidate_all()
                    
                    send_alert("🛡️ [Watchdog] 청산 완료. 계좌 깡통을 방어했습니다. 시장 안정화 및 파라미터 점검을 위해 24시간 셧다운 모드에 진입합니다.")
