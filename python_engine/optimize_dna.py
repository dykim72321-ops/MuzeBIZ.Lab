...
def run_optimization():
    tickers = [
        "SOFI", "PLTR", "AMC", "GME", "NIO", "PGR", "HOOD", "AFRM",
        "UPST", "COIN", "MARA", "RIOT", "FSR", "NKLA", "LCID", "DKNG"
    ]
    ...
    benchmark_data = yf.download("IWM", start=base_validator.start_date, end=base_validator.end_date)["Close"]
        ...
        for tgt in targets:
            ...
            for ticker in tickers:
                if ticker in preprocessed_base:
                    trades = validator.simulate_ticker(ticker, preprocessed_base[ticker])
                    if trades:
            ...
            print(f"  └ 결과: PnL={avg_pnl:.2f}%, Win={win_rate:.1f}%, PF={pf:.2f}, Trades={len(all_trades)}")
