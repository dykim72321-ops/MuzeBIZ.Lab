...
MAX_CONCURRENT_POSITIONS = 15  # 동시 보유 최대 종목 수 (실질 도달 가능한 상한)
MAX_CONCENTRATION_PCT = 0.75   # 총 자산 대비 투입 비중 상한 (15종목 × $5k = $75k = 75%)
TS_INIT_PCT    = 0.93   # 일반: 초기 TS -7% (ATR 스탑에 유연성을 부여하기 위해 하드스탑 완화)
...
1. **신규 진입**: 현재 포지션이 없고, 보유 중인 종목 수가 15개(MAX_CONCURRENT_POSITIONS) 미만이며, 투입 자본이 계좌의 75%(MAX_CONCENTRATION_PCT) 미만일 때 진입. 페니 주식은 DNA≥60, 일반 주식은 DNA≥75 필요.
