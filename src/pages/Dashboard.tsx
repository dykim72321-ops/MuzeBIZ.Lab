...
const NormalSignalMiniCard = ({ rawData }: { rawData: any }) => {
  ...
  return (
    <div className="flex items-center justify-between px-6 py-4 bg-[#0b101a]/80 border border-slate-800 rounded-xl hover:border-indigo-500/50 transition-all group shadow-sm">
      <div className="flex items-center gap-4">
        <div className="w-1 h-10 bg-slate-700 rounded-full group-hover:bg-indigo-500 transition-colors" />
        <div>
          <span className="text-lg font-black text-white tracking-tight">{rawData.ticker}</span>
      ...
      <div className="flex items-center gap-6">
        <div className="text-right">
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.15em]">RSI</p>
          <p className="font-mono font-black text-base text-slate-300">{rawData.rsi?.toFixed(1) ?? '—'}</p>
        </div>
        <div className={clsx(
        ...
        <span className="text-[9px] font-black text-slate-500 bg-slate-800/50 border border-slate-700 px-3 py-1.5 rounded-md tracking-[0.2em] uppercase">
          Standard
        </span>
      </div>
    </div>
...
export const Dashboard = () => {
  ...
  return (
    <div className="bg-[#020617] min-h-screen relative overflow-hidden">
      ...
      {/* Content */}
      <div className="relative z-10 max-w-[1600px] mx-auto px-6 py-8 space-y-8 animate-in fade-in duration-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#0b101a]/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-2xl flex items-center gap-5">
            <div className="p-3.5 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
              <div className="w-7 h-7 text-emerald-400 font-black flex items-center justify-center text-xs">MDD</div>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.15em] mb-1">시스템 방어력</p>
              <div className="flex items-baseline gap-2">
                ...
                {!statsLoading && <span className="text-xs font-bold text-emerald-400">vs MKT -29.1%</span>}
              </div>
              <p className="text-[10px] text-slate-500 mt-1 font-medium">퀀트 엔진 백테스트 기준</p>
            </div>
          </div>

          <div className="bg-[#0b101a]/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-2xl flex items-center gap-5">
            <div className="p-3.5 bg-indigo-500/10 rounded-xl border border-indigo-500/20">
              <div className="w-7 h-7 text-indigo-400 font-black flex items-center justify-center text-xs">WIN</div>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.15em] mb-1">엔진 승률</p>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-black text-white tabular-nums">
                  {statsLoading
                    ? <span className="text-sm font-bold text-indigo-400 animate-pulse">CALCULATING...</span>
          ...
          <div className="bg-[#0b101a]/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-2xl flex items-center gap-5">
            <div className="p-3.5 bg-blue-500/10 rounded-xl border border-blue-500/20">
              <div className="w-7 h-7 text-blue-400 font-black flex items-center justify-center text-xs">LIVE</div>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.15em] mb-1">실시간 포착 종목</p>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-black text-white tabular-nums">{buyTickers.length}</span>
              ...
              <p className="text-[10px] text-slate-500 mt-1 font-medium font-mono uppercase tracking-tighter">
        ...
        <div className="pt-4">
          ...
          {/* Empty State */}
          {!hasAnySignal && (
            <div className="flex flex-col items-center justify-center h-80 bg-[#0b101a]/40 border-2 border-dashed border-slate-800 rounded-2xl text-slate-400 shadow-inner">
              <div className="w-16 h-16 bg-slate-900 rounded-full flex items-center justify-center mb-4">
                <Server className="w-8 h-8 opacity-30" />
              </div>
              <p className="font-bold text-slate-200">수신된 펄스 시그널이 없습니다.</p>
              <p className="text-sm mt-1 text-slate-400">상단의 '하이브리드 헌팅 트리거'를 눌러 시장 스캔을 시작하세요.</p>
            </div>
          )}
                ...
                {displayedStrong.map((ticker) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={clsx(
                        "flex flex-col gap-5 bg-[#0b101a]/80 border border-slate-800 rounded-2xl p-6 shadow-sm transition-all hover:border-indigo-500/50",
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h3 className="text-3xl font-black tracking-tighter text-white">{ticker}</h3>
                            <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mt-0.5">Quantum Buy Signal</p>
                          </div>
                        </div>
                        <span className="px-3 py-1.5 rounded-md text-sm font-black bg-slate-800 text-slate-200 border border-slate-700 shadow-lg">
          ...
          {displayedNormal.length > 0 && (
            <section className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black text-slate-400 bg-slate-800/50 border border-slate-700 px-2.5 py-0.5 rounded-full">
          ...
          {hasAnySignal && displayedStrong.length === 0 && displayedNormal.length === 0 && (
            <div className="flex flex-col items-center justify-center h-40 bg-[#0b101a]/40 border border-dashed border-slate-800 rounded-2xl text-slate-400">
              <p className="font-bold text-slate-400">현재 해당 강도의 시그널이 없습니다.</p>
            </div>
          )}
        </div>
      </div>
