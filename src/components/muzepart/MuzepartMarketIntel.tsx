...
export const MuzepartMarketIntel: React.FC<MarketIntelProps> = ({ intelData, resultsCount }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch mb-6">
      ...
      <div className="lg:col-span-4 bg-white border border-blue-200/85 p-6 rounded-2xl shadow-sm flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between mb-6 pb-2 border-b border-blue-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-black text-blue-900 uppercase tracking-wider">Global Inventory</h3>
                <p className="text-xs text-blue-650 font-bold">Distribution by Region/Dealer</p>
              </div>
            </div>
          </div>
          <div className="h-[200px] flex items-center justify-center">
            {!intelData ? (
              <p className="text-xs text-blue-500 font-semibold">검색 후 업데이트됩니다.</p>
            ) : (
      ...
      <div className="lg:col-span-4 bg-white border border-blue-200/85 p-6 rounded-2xl shadow-sm flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between mb-6 pb-2 border-b border-blue-100">
            <div className="flex flex-col">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-blue-900 uppercase tracking-wider">Supply Risk Index</h3>
                  <p className="text-xs text-blue-650 font-bold">Live Procurement Stability Map</p>
                </div>
              </div>
            </div>
          </div>
          <div className="h-[200px] flex flex-col justify-center gap-4 px-2">
            {!intelData ? (
              <p className="text-xs text-center text-blue-500 font-semibold">검색 후 업데이트됩니다.</p>
            ) : (
              <>
                <div className="relative">
                  <div className="flex justify-between text-xs font-bold mb-1">
                    <span className="text-rose-600">재고 부족 (High Risk)</span>
                    <span className="text-blue-700 font-mono">{intelData.riskData.find((d: any) => d.name === 'High')?.value || 0} 개 판매처</span>
                  </div>
                  <div className="h-2 bg-blue-100 rounded-full overflow-hidden border border-blue-250">
                  ...
                  <p className="text-xs text-blue-500 mt-1">재고 0개 (품절 또는 EOL 의심)</p>
                </div>
                <div className="relative">
                  <div className="flex justify-between text-xs font-bold mb-1">
                    <span className="text-amber-600">재고 한정 (Medium Risk)</span>
                    <span className="text-blue-700 font-mono">{intelData.riskData.find((d: any) => d.name === 'Medium')?.value || 0} 개 판매처</span>
                  </div>
                  <div className="h-2 bg-blue-100 rounded-full overflow-hidden border border-blue-250">
                  ...
                  <p className="text-xs text-blue-500 mt-1">재고 1~100개 (수급 주의)</p>
                </div>
                <div className="relative">
                  <div className="flex justify-between text-xs font-bold mb-1">
                    <span className="text-emerald-600">재고 안정 (Low Risk)</span>
                    <span className="text-blue-700 font-mono">{intelData.riskData.find((d: any) => d.name === 'Low')?.value || 0} 개 판매처</span>
                  </div>
                  <div className="h-2 bg-blue-100 rounded-full overflow-hidden border border-blue-250">
                  ...
                  <p className="text-xs text-blue-500 mt-1">재고 100개 초과 (수급 원활)</p>
                </div>
              </>
            )}
          </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-6 pb-2 border-b border-blue-100">
            <div className="flex flex-col">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-blue-900 uppercase tracking-wider">Market Price Drift</h3>
                  <p className="text-xs text-blue-650 font-bold">Cross-Distributor Price Benchmark</p>
                </div>
              </div>
            </div>
          </div>
          <div className="h-[200px]">
            {!intelData ? (
              ...
              <div className="h-full flex items-center justify-center">
                <p className="text-xs text-blue-500 font-semibold">가격 정보가 없습니다.</p>
              </div>
            ) : (
              <div className="h-full flex flex-col">
                ...
                {intelData.priceStats && (
                  <div className="flex items-center justify-between text-xs font-bold text-blue-600 px-1 pt-2.5 border-t border-blue-100">
                    <span>Min: <span className="text-emerald-600 font-mono">${intelData.priceStats.min.toLocaleString()}</span></span>
                    <span>Avg: <span className="text-blue-800 font-mono">${intelData.priceStats.avg.toFixed(2)}</span></span>
