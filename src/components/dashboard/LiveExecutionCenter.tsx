...
export const LiveExecutionCenter = () => {
  ...
  if (loading) return (
    <div className="h-[600px] bg-white rounded-[2.5rem] border border-slate-200 shadow-sm flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <Activity className="w-8 h-8 text-indigo-500 animate-pulse" />
        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Establishing Tactical Link...</span>
      </div>
    </div>
  );

  return (
    <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden flex flex-col min-h-[850px] shadow-sm relative">
      {/* 1. TOP CONTROL STRIP */}
      <div className={clsx(
        "p-6 border-b border-slate-200 relative z-20 transition-all duration-700",
        isArmed ? "bg-rose-50" : "bg-slate-50"
      )}>
          ...
          <div className="flex items-center gap-6">
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 px-2 py-1 bg-white rounded-md border border-slate-200 shadow-sm">
                  <div className={clsx("w-1.5 h-1.5 rounded-full", brokerConnected ? "bg-emerald-500 animate-pulse" : "bg-rose-500")} />
                  <span className="text-[10px] font-black text-slate-800 uppercase tracking-tight">Broker Sync</span>
                </div>
              </div>
            </div>

            <div className="flex flex-col min-w-[120px]">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Base Lot</span>
              <div className="flex items-center bg-white rounded-lg border border-slate-200 px-2 py-1 shadow-sm">
                <input
                  type="number"
                  value={lotSize}
                  onChange={(e) => setLotSize(Number(e.target.value))}
                  className="bg-transparent text-xs font-black text-slate-900 w-12 outline-none border-none text-center"
                />
                <span className="text-[9px] font-bold text-slate-400 ml-1 font-mono">USD</span>
              </div>
            </div>
          </div>
      ...
      <div className="p-8 grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-50 border border-slate-200 p-6 rounded-3xl shadow-sm">
            ...
            <p className="text-3xl font-black text-slate-900 tabular-nums tracking-tighter">${(account?.cash_available ?? 0).toLocaleString()}</p>
          </div>
          <div className="bg-slate-50 border border-slate-200 p-6 rounded-3xl shadow-sm">
      ...
      <div className="bg-slate-50 border-b border-slate-200 p-6 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">Execution Matrix</h2>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden">
        <div className="lg:col-span-8 overflow-y-auto p-6 border-r border-slate-200 custom-scrollbar">
            ...
            <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
              {activeTab === 'positions' && (
                positions.length > 0 ? positions.map((pos) => (
                  <div key={pos.ticker} className="bg-slate-50 border border-slate-200 rounded-2xl p-6 flex justify-between items-center group hover:bg-white hover:shadow-sm transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-3">
                          <span className="text-xl font-black text-slate-900">{pos.ticker}</span>
                        ...
                        <div className={clsx("text-[10px] font-black mt-0.5", (pos.unrealized_pl || 0) >= 0 ? "text-emerald-600" : "text-rose-600")}>
                          ...
                          {pos.is_paper && <span className="ml-2 text-[8px] bg-slate-100 px-1 py-0.5 rounded text-slate-500 border border-slate-200">QUANT_PAPER</span>}
                        </div>
                      </div>
                    </div>
              ...
              {activeTab === 'signals' && (
                signals.length > 0 ? signals.map((sig: any) => (
                  <div key={sig.id || sig.ticker} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex justify-between items-center group hover:bg-white hover:shadow-sm transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-base font-black text-slate-900">{sig.ticker}</span>
                  ...
                  <div className="py-20 text-center text-slate-400 text-[10px] font-black uppercase tracking-widest">No active signals</div>
                )
              )}
              ...
              {activeTab === 'history' && (
                history.length > 0 ? history.map((order: any) => {
                  ...
                  return (
                    <div key={order.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex justify-between items-center">
                      <div className="flex items-center gap-4">
                        <div className={clsx("w-10 h-10 rounded-2xl flex items-center justify-center border", isProfit ? "bg-emerald-50 border-emerald-200" : "bg-rose-50 border-rose-200")}>
                          {order.status === 'filled' ? <CheckCircle className="w-4 h-4 text-emerald-600" /> : <XCircle className="w-4 h-4 text-slate-400" />}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-base font-black text-slate-900">{order.ticker}</span>
                      ...
                      <div className="text-right">
                        <div className={clsx("text-[10px] font-black uppercase px-2 py-0.5 rounded", order.status === 'filled' ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-slate-100 text-slate-500")}>{order.status}</div>
                        <div className="text-[9px] text-slate-400 mt-1">{order.created_at ? new Date(order.created_at).toLocaleString() : '--'}</div>
                      </div>
                    </div>
                  );
                }) : (
                  <div className="py-20 text-center text-slate-400 text-[10px] font-black uppercase tracking-widest">No order history</div>
                )
              )}
            </motion.div>
          </AnimatePresence>
        ...
        <div className="lg:col-span-4 bg-slate-50 p-6 flex flex-col border-l border-slate-200">
          <div className="flex items-center gap-2 text-slate-500 mb-6 border-b border-slate-200 pb-4 text-[10px] font-black uppercase tracking-[0.3em]">
            <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse" />
            Live Order Broadcast
          </div>
          <div className="flex-1 font-mono text-[11px] space-y-3 overflow-y-auto custom-scrollbar leading-relaxed">
            <div className="text-slate-400">[{new Date().toLocaleTimeString()}] NODE_READY: Autonomous Engine active.</div>
            <div className="text-slate-400">[{new Date().toLocaleTimeString()}] MD_SYNC: Alpaca stream connected.</div>
            ...
            <div className="text-slate-300 mt-4 italic tracking-widest">// Monitoring execution signal matrix...</div>
          </div>
        </div>
      </div>
