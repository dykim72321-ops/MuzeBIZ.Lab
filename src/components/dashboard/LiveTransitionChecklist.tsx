...
export const LiveTransitionChecklist = () => {
  ...
  return (
    <div className="sfdc-card flex flex-col">
      <div className="p-6 pb-4 bg-white border-b border-slate-100 flex justify-between items-end">
        <div>
          <h2 className="text-sm font-black text-slate-900 flex items-center gap-2">
            <ShieldCheck className="w-5 h-5 text-slate-900" />
            실계좌 전환 체크리스트
          </h2>
          <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">Live Trading Readiness</p>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-[10px] font-black text-slate-400 font-mono uppercase tracking-widest mb-0.5">Score</span>
          <span className={clsx("transition-colors text-lg font-black leading-none", isAllCleared ? "text-emerald-500" : "text-slate-900")}>
      ...
      <div className="p-5 bg-slate-50 border-t border-slate-100 rounded-b-2xl">
        <button
          ...
          className={clsx(
            "w-full font-black transition-all text-xs",
            isAllCleared
              ? "sfdc-button-primary shadow-xl py-3.5 rounded-xl"
              : "py-3.5 rounded-xl bg-slate-200 text-slate-400 cursor-not-allowed"
          )}
        >
