...
export const Layout = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-slate-800 font-sans selection:bg-blue-500/20 selection:text-blue-800">
      ...
      <div className="lg:hidden fixed bottom-4 left-4 right-4 h-16 bg-white/95 backdrop-blur-xl border border-slate-200 rounded-2xl flex items-center justify-around px-2 z-50 shadow-md">
        <NavLink to="/stock/dashboard" className={({ isActive }) => clsx("flex flex-col items-center gap-1 px-3 py-1 rounded-lg transition-colors relative z-10", isActive ? "text-blue-600 bg-blue-50" : "text-slate-500 hover:text-slate-800")}>
          <LayoutDashboard className="w-5 h-5" />
          <span className="text-[10px] font-bold">작전지휘소</span>
        </NavLink>
        <NavLink to="/parts-search" className={({ isActive }) => clsx("flex flex-col items-center gap-1 px-3 py-1 rounded-lg transition-colors relative z-10", isActive ? "text-blue-600 bg-blue-50" : "text-slate-500 hover:text-slate-800")}>
